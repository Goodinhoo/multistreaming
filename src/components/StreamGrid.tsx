...
export function StreamGrid({
  ...
  const [platformStreamInfo, setPlatformStreamInfo] = useState<
    Record<
      string,
      {
        platform: Platform;
  ...
  const streamerPlatformsKey = useMemo(() => {
    return activeStreamers
      .map(
        (s) =>
          `${s.id}-${s.platforms.twitch || ''}-${s.platforms.kick || ''}-${
            s.platforms.youtube || ''
          }`
      )
      .join('|');
      ...
      >
        {activeStreamers.length === 0 ? (
          ...
          >
            ...
            >
              Adicione streamers e aguarde eles ficarem online para ver os
              streams aqui
            </p>
            <div
              style={{
          ...
          >
            {activeStreamers.map((streamer) => (
              ...
              >
                  ...
                  >
                    {streamer.platformCount} plataforma
                    {streamer.platformCount !== 1 ? 's' : ''}
                  </div>

                  {streamer.status === 'online' &&
                  activePlatforms[streamer.id] &&
                  streamer.platforms[activePlatforms[streamer.id]] ? (
                    <>
                      <iframe
                        ...
                        onLoad={() => {
                          ...
                          if (activePlatforms[streamer.id] === 'twitch') {
                            try {
                              ...
                              console.log(
                                'Erro ao tentar iniciar autoplay:',
                                error
                              );
                            }
                          }
                  ...
                  >
                    <div
                      style={{
                        ...
                        marginBottom: '1rem',
                        filter:
                          'drop-shadow(0 0 20px rgba(107, 114, 128, 0.5))',
                      }}
                    >
                      ⚫
                    </div>
                    ...
                    >
                      Aguardando streamer ficar online
                    </p>
                  </div>
                </div>
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      const platformKey = activePlatform
                        ? `${streamer.id}-${activePlatform}`
                        : null;
                      const currentStreamInfo =
                        platformKey && platformStreamInfo[platformKey]
                    ...
                    >
                      ...
                      {onToggleNotifications && (
                        <button
                          onClick={(e) => {
                            ...
                            showSuccessToast(
                              'Notificações',
                              `Notificações para ${streamer.name} foram ${
                                wasEnabled ? 'desativadas' : 'ativadas'
                              }!`
                            );
                          }}
                          style={{
                            ...
                            justifyContent: 'center',
                            cursor: 'pointer',
                            color: streamer.notificationsEnabled
                              ? '#93c5fd'
                              : '#3b82f6',
                      ...
                      {onToggleFavorite && (
                        ...
                        >
                          <Heart
                            size={16}
                            fill={streamer.isFavorite ? 'currentColor' : 'none'}
                          />
                        </button>
                      )}
                    </div>
                  ...
                  >
                    ...
                    >
                      {(() => {
                        ...
                        return (
                          <>
                            {currentStreamInfo?.viewers !== undefined &&
                              currentStreamInfo.viewers > 0 && (
                                <span
                                  style={{
                                    fontSize: '0.875rem',
                                    color: '#10b981',
                                ...
                                >
                                  ...
                                  />
                                  {currentStreamInfo.viewers.toLocaleString()}{' '}
                                  viewers
                                </span>
                              )}
                    ...
                    >
                      ...
                      {getAvailablePlatforms(streamer).map((platform) => {
                        const isActive =
                          activePlatforms[streamer.id] === platform;
                        return (
                          <button
                            key={platform}
                            onClick={(e) => {
